...
function createGame(data) {
	...
	let className = window.__location.pathname == "/pinned.html" ? " big" : "";
	let gameElm = document.createElement("div");
	gameElm.innerHTML = `<div data-game="${data.name}" class="widget game${className}" onclick="${evt}"><p>${capitalizeString(data.name)}</p><img src="${window.__location.origin + data.data.icon}"/></div>`;
...
function load() {
	document.querySelector(".addGame .add input").addEventListener("keyup", async (evt) => {
		if (evt.key == "Enter") {
			...
			games[titleText.toLowerCase().slice(0,11)] = {
				icon: "/images/web.png",
				url: url
			}
			localStorage.setItem("__roguecustom", JSON.stringify(games));
			window.__location.reload(false);
		}
	})
...
window.onload = () => {
	async function loadBare(){
		window.__bareClient = await createBareClient(window.__location.origin+"/bare/");
	}
	
	if (window.__location.pathname == "/games.html") {
		loadBare();
		gamePage();
		load();
	}
	if (window.__location.pathname == "/pinned.html") pinnedPage();
